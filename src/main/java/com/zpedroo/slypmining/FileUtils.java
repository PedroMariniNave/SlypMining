...
public class FileUtils {
    ...
    private SlypMining slypMining;
    private Map<Files, FileManager> files;

    public FileUtils(SlypMining slypMining) {
        instance = this;
        this.slypMining = slypMining;
    ...
    public enum Files {
        ...
        REWARDS("rewards", "yml", "menus", "menus", false),
        KEY("key", "yml", "menus", "menus", false),
        BASIC("basic", "yml", "keys", "keys", true);
    ...
    public class FileManager {
        ...
        public FileManager(Files file) {
            this.pdfile = new File(slypMining.getDataFolder() + (file.getFolder().isEmpty() ? "" : "/" + file.getFolder()), file.getName() + '.' + file.getExtension());

            if (!pdfile.exists()) {
                if (file.requireEmpty) {
                    File folder = new File(slypMining.getDataFolder(), "/machines");
                ...
                try {
                    ...
                    copy(slypMining.getResource((file.getResource().isEmpty() ? "" : file.getResource() + "/") + file.getName() + '.' + file.getExtension()), pdfile);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
